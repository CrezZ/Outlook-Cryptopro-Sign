Attribute VB_Name = "CryptoProSignAttachAndReply"
'---------------------------------------------------------------------------------
...
'''CHANGE CERTIFICATE
        If MsgBox(MsgRequestAttached, vbYesNoCancel) = vbYes Then
        
          SaveSetting "CryptcpOutlook", "Startup", "AttachedSign", "-signf" ''deattached sign
        Else
        If MsgBox(MsgRequestAttached, vbYesNoCancel) = vbNo Then
